...
func init() {
	flag.BoolVar(&logging.toStderr, "tlogtostderr", false, "log to standard error instead of files")
	flag.BoolVar(&logging.alsoToStderr, "talsologtostderr", false, "log to standard error as well as files")
	flag.Var(&logging.verbosity, "tv", "log level for V logs")
	flag.Var(&logging.stderrThreshold, "tstderrthreshold", "logs at or above this threshold go to stderr")
	flag.Var(&logging.vmodule, "tvmodule", "comma-separated list of pattern=N settings for file-filtered logging")
	flag.Var(&logging.traceLocation, "tlog_backtrace_at", "when logging hits line file:N, emit a stack trace")
